...
function SignatureCase({ caseItem, index }: { caseItem: Case; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-card transition-all duration-normal ease-editorial hover:border-accent/40 hover:shadow-elevated"
    >
      ...
      <div className="flex flex-col p-5 lg:p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="font-heading text-2xl font-bold leading-none text-primary/15 transition-colors group-hover:text-primary/25">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {caseItem.tags.slice(0, 4).map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-border bg-secondary/70 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-primary"
            >
              {tag}
            </span>
          ))}
