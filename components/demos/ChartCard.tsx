    ...
    async function renderChart() {
      ...
      const option = chart.kind === "donut"
        ...
        : {
            ...
            legend: chart.kind === "grouped-bar"
              ? { top: 0, textStyle: { color: muted, fontFamily: "Inter", fontSize: 11 } }
              : undefined,
            grid: { top: chart.kind === "grouped-bar" ? 42 : 20, right: 18, bottom: rotateLabels ? 58 : 42, left: 56, containLabel: true },
            xAxis: { type: "category", data: labels, ...baseAxis, axisLabel: { ...baseAxis.axisLabel, interval: 0, rotate: rotateLabels } },
            yAxis: {
              type: "value",
              ...baseAxis,
              axisLabel: {
            ...
            series: chart.kind === "grouped-bar"
              ? [
                  { type: "bar", name: chart.series?.[0] ?? "Principal", data: values, barMaxWidth: 24, itemStyle: { borderRadius: [5, 5, 0, 0] } },
                  { type: "bar", name: chart.series?.[1] ?? "Secundário", data: chart.data.map((item) => item.secondary ?? 0), barMaxWidth: 24, itemStyle: { borderRadius: [5, 5, 0, 0] } },
                ]
              : [{ type: "bar", name: chart.title, data: values, barMaxWidth: 32, itemStyle: { borderRadius: [6, 6, 0, 0] }, markLine: chart.reference ? { symbol: "none", lineStyle: { color: warm, type: "dashed" }, label: { color: warm, formatter: `referência ${chart.reference}` }, data: [{ yAxis: chart.reference }] } : undefined }],
          };
    ...
    <article className="demo-panel">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full bg-editorial px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-muted-foreground">
          {chart.unit === "BRL" ? "R$" : chart.unit === "PERCENT" ? "%" : chart.unit === "KM" ? "km" : "volume"}
        </span>
      </div>
      <div ref={chartElement} className="mt-4 h-[260px] w-full" role="img" aria-label={`${chart.title}: ${chart.data.map((item) => `${item.label} ${valueLabel(item.value, chart.unit)}`).join(", ")}`} />
    </article>
  );
}
