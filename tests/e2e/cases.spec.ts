...
test.describe("Cases e biblioteca", () => {
  ...
  test("filtros da biblioteca atualizam a lista de cases", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Frete e Custo/ })).toHaveCount(0);
    await expect(page.getByRole("button", { name: /Operação de CD/ })).toHaveCount(0);
    ...
    await expect(page.getByRole("button", { name: /^Roteirização e SLA/ }).first()).toHaveAttribute(
      "aria-pressed",
      "true",
    );

    await expect(page.getByRole("button", { name: /^Todos/ }).first()).toHaveAttribute(
      "aria-pressed",
      "false",
    );
  ...
  test("CTAs dos cases não estouram cards nem sobrepõem métricas", async ({ page }) => {
    for (const width of [390, 768, 1024, 1280, 1366, 1440]) {
      ...
      const overflow = await page.evaluate(() => {
        ...
        for (const card of document.querySelectorAll(
          ...
          const title = card.querySelector("h3")?.textContent?.trim() ?? "case sem título";
        ...
        for (const item of document.querySelectorAll('[data-testid="case-library-item"]')) {
          ...
          const title = item.querySelector("h3")?.textContent?.trim() ?? "case de biblioteca";
