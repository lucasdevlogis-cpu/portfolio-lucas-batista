...
def add_points(
    ...
    for _, row in df.iterrows():
        ...
        if color_by and color_by in row:
            ...
            cor = brand.STATUS_COLORS.get(key) or brand.SEVERITY_COLORS.get(key) or brand.PRIMARY
            folium.CircleMarker(
    ...
    for i, r in enumerate(routes):
        ...
        if show_numbers:
            add_numbered_markers(
                ...
                labels=[
                    str(j + 1)
                    for j in range(len(coords) - 2 if depot and len(coords) > 2 else len(coords))
                ],
                color=color,
            )
        else:
