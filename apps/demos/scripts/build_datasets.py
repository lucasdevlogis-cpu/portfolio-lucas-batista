...
def frete_embarques(n: int = 120) -> None:
        ...
        rows.append(
            {
                ...
                "gross_weight_kg": round(
                    max(
                        20,
                        _jitter(seed["gross_weight_kg"], seed["gross_weight_kg"] * 0.25),
                    ),
                    1,
                ),
                "volume_m3": round(
                ...
                "invoice_value_brl": round(
                    max(
                        500,
                        _jitter(seed["invoice_value_brl"], seed["invoice_value_brl"] * 0.3),
                    ),
                    2,
                ),
    ...
    pd.DataFrame(rows).to_csv(GENERATED / "frete_embarques.csv", index=False, encoding="utf-8")
...
def cvrp_entregas(n: int = 40) -> None:
    ...
    pd.DataFrame(rows).to_csv(GENERATED / "cvrp_entregas.csv", index=False, encoding="utf-8")
...
def vrptw_paradas(n: int = 12) -> None:
    ...
    pd.DataFrame(rows).to_csv(GENERATED / "vrptw_paradas.csv", index=False, encoding="utf-8")
...
def corredores_geo() -> None:
    ...
    pd.DataFrame(rows).to_csv(GENERATED / "corredores_geo.csv", index=False, encoding="utf-8")


def ship_from_store(n_pedidos: int = 60) -> None:
    origens = pd.read_csv(RAW / "origens_sfs.csv", encoding="utf-8")
    origens.to_csv(GENERATED / "sfs_origens.csv", index=False, encoding="utf-8")
    ...
    pd.DataFrame(rows).to_csv(GENERATED / "sfs_pedidos.csv", index=False, encoding="utf-8")
        ...
        rows.append(
            {
                ...
                "custo_medio_frete": round(float(max(12, _jitter(25 + prazo_base * 4, 10))), 2),
                "volume_entregas": int(rng.integers(20, 600)),
            }
        )
    pd.DataFrame(rows).to_csv(GENERATED / "promessa_cep.csv", index=False, encoding="utf-8")
...
def enderecos(n: int = 60) -> None:
    ...
    for i in range(n):
        ...
        rows.append(seed)
    pd.DataFrame(rows).to_csv(GENERATED / "enderecos.csv", index=False, encoding="utf-8")


def tsp_visits() -> None:
    base = pd.read_csv(RAW / "tsp_visits.csv", encoding="utf-8")
    base.rename(columns={"latitude": "lat", "longitude": "lon"}).to_csv(
        GENERATED / "tsp_visits.csv", index=False, encoding="utf-8"
    )
...
def main() -> None:
    ...
    torre_entregas()
    ocorrencias()
    print("Datasets construídos em", GENERATED)
