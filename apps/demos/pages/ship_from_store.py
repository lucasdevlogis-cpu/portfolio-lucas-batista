...
ui.hero(
    ...
    selo=brand.maturidade(metodo="score custo+prazo", producao="OMS integrado a WMS/ERP"),
    metric={
...
with tab_analise:
    ...
    with col1:
        ...
        fig2.update_traces(hovertemplate=fmt.fmt_hover([("Origem", "%{x}"), ("Pedidos", "%{y}")]))
    ...
    with col2:
        ...
        fig3.update_traces(
            hovertemplate=fmt.fmt_hover([("UF", "%{x}"), ("Economia", "R$ %{y:,.2f}")])
        )
