...
from domain import freight as frete
...
with tab_visao:
    ...
    with col_map:
        ...
        edges = []
        fluxo = calc.groupby(["origin_uf", "destination_uf"])["frete_total"].sum().reset_index()
        ...
        m = folium_maps.base_map(
            center=(-18, -47), zoom=4, height=ui.map_height(brand.MAP_FULL_HEIGHT)
        )
        if edges:
    ...
    with col_wf:
        ...
        hover_comp = fmt.fmt_hover(
            [(c, fmt.fmt_currency(v)) for c, v in zip(componentes, valores, strict=True)]
        )
        wf = go.Figure(
            go.Waterfall(
                orientation="v",
...
with tab_analise:
    ...
    totais = []
    for p in precos:
        ajuste = sum(frete.diesel_delta(frete_peso, p) for frete_peso in calc["Frete-peso"])
...
with tab_exportar:
    ...
    tabela["status_piso"] = tabela["acima_piso_pct"].apply(
        lambda x: (
            tables.status_text("Acima do piso") if x >= 0 else tables.status_text("Abaixo do piso")
        )
    )
