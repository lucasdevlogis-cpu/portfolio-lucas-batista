...
if base.empty:
    ui.hero(
        ...
        selo=brand.maturidade(metodo="custo paramétrico", producao="malha real + pedágio vigente"),
        metric={
...
with tab_visao:
    ...
    m = fm.add_legend(
        ...
        [
            {
                "color": brand.PRIMARY,
                "label": f"Maior ({fmt.fmt_number(max_vol, decimals=0)} t) — linha grossa",
            },
            {
                "color": brand.PRIMARY,
                "label": f"Menor ({fmt.fmt_number(min_vol, decimals=0)} t) — linha fina",
            },
        ],
